...
inline int avp_str_value(struct avp *avp, unsigned char **val){
	...
	CHECK_FCT_DO( fd_msg_avp_hdr( avp, &hdr ), return 1; );
...
inline int avp_int_value(struct avp *avp,  int32_t *val){
...
inline int avp_uint_value(struct avp *avp, uint32_t *val){
...
inline int get_str_value(struct msg *msg, struct dict_object *dict, unsigned char **val){
...
inline int get_int_value(struct msg *msg, struct dict_object *dict,  int32_t *val){
...
inline int get_str_value_gavp(struct avp *gavp, struct dict_object *dict, unsigned char **val){
	...
	fd_avp_search_avp ( gavp, dict, &avp, MSG_BRW_FIRST_CHILD );
	if (avp) 
		return avp_str_value(avp, val);
	return 1;	
}
...
/*Retrieves string value of the first avp indicated by 'dict' with in group avp 'gavp' and store the resulting pointer to 'target'*/
inline int get_int_value_gavp(struct avp *gavp, struct dict_object *dict,  int32_t *val){
...
inline int get_str_array(struct msg * msg, struct dict_object * dict, unsigned char *** array_val, size_t *array_size){

	struct avp * avp, *avp2;
	unsigned char ** temp;
	size_t num = 0;

	if((!msg) || (!array_val) || (!dict) || (!array_size))
			return 1;
	...
	*array_size = 0;
	...
	while(avp){
		...
		if((temp = realloc(temp, sizeof(char *) * (num + 1))) == NULL){
			...
			fprintf(stderr, "Error : Memory allocation failed in get_str_array.\n");
			return 1;
		}
...
inline int get_str_gavp_array(struct avp * gavp, struct dict_object * dict, unsigned char *** array_val, size_t *array_size){

	struct avp * avp, *avp2;
	unsigned char ** temp = NULL;
	size_t num = 0;

	if((!gavp) || (!dict) || (!array_val) || (!array_size))
			return 1;
...
inline int get_int_gavp_array(struct avp * gavp, struct dict_object * dict, int32_t ** array_val, size_t *array_size){
	...
	*array_size = 0;
	...
	/*search for the first child AVP*/
	fd_avp_search_avp ( gavp, dict, &avp, MSG_BRW_FIRST_CHILD );
	while(avp){

		if((temp = realloc(temp, sizeof(int32_t) * (num + 1))) == NULL){
...
inline int get_uint_array(struct msg * msg, struct dict_object * dict, uint32_t ** array_val, size_t *array_size){

	struct avp * avp, *avp2;
	uint32_t * temp;
	size_t num = 0;
	
	if((!msg) || (!dict) || (!array_val) || (!array_size))
			return 1;
