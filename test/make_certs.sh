...
#peer2
openssl genrsa -out peer3.key.pem 1024
openssl req -new -batch -out peer3.csr.pem -key peer3.key.pem -subj /CN=peer3.localdomain/C=FR/ST=BdR/L=Aix/O=fD/OU=Tests
openssl ca -cert cacert.pem -keyfile cakey.pem -in peer3.csr.pem -out peer3.cert.pem -outdir . -batch
